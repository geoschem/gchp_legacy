...
!BOP
!
! !ROUTINE:  GFIO_remap --- Remaping the data.
!
!
...
!                              -----------------------
      ...
      real, pointer     :: pe3(:,:,:)          ! Output Pressure edges
      real, pointer     :: pe0(:,:)            ! Input Pressure edges (im,kn)
      real, pointer     :: pe1(:,:)            ! Input Pressure edges (im,kn)
      real, pointer     :: pe2(:,:)            ! Input Pressure edges (im,kn)
      real, pointer     :: pe4(:,:)            ! output Pressure edges (im,kn)
      real, pointer     :: pk0(:,:)            ! Input p ** kappa  (im,km)
      real, pointer     :: pk1(:,:)            ! Output p ** kappa (im,kn)
      real, pointer     :: ps(:,:)             ! Surface Pressure
      real, pointer     :: delp(:,:,:)         ! Pressure delta
...
!    Get meta data
     ...
     if(lat_e(1) > -90.0 .and. lat_e(jm_e) < 90.0) then
       ...
       if(associated(delp)) deallocate(delp)
       allocate(delp(im_e,jm_e,km_e))
       if(associated(lat_e2)) deallocate(lat_e2)
...
!       print *,' nVars, nVars_e ', nVars, nVars_e
      ...
      if(kn /= km_e) then
       if(gaussian == 0) then
        if(associated(pe3d_m)) deallocate(pe3d_m)
...
!         ---------------------------------------------------------
          ...
          if(associated(ak32)) deallocate(ak32)
          allocate(ak32(kn1+1))
          if(associated(bk32)) deallocate(bk32)
          allocate(bk32(kn1+1))
...
!!!          print *,' out from set_eta ks32'
        ...
        if(kn /= km_e) then
          ...
          if(associated(dpref)) deallocate(dpref)
          if(associated(pe2)) deallocate(pe2)
          if(associated(pk1)) deallocate(pk1)
          if(associated(pe4)) deallocate(pe4)
          if(associated(pe3)) deallocate(pe3)
          if(associated(pe1)) deallocate(pe1)
...
!        ---------------------------------------------------------
         ...
         do iv = 1, nVars_e
          if ( vname(iv) == 'SURFP' .or. vname(iv) == 'PS' .or. &
            ...
            ps_found = .true.
            if(associated(gaus_ps)) deallocate(gaus_ps)
          ...
          if ( vname(iv) == 'DELP' .or. vname(iv) == 'delp' ) then
           delp_found = .true.
            if(associated(gaus_delp)) deallocate(gaus_delp)
...
!!!               print *,' jm_e, jme_tmp ',jm_e,jme_tmp
               if(associated(inField)) deallocate(inField)
...
!
               if(gaussian == 0) then
                 ...
                 if ( rc2 /= 0 )  call die (myname, 'can not allocate gaus_inField ')
...
!                      call minmaxpe(v,im_e,jme_new,kn_e)
               ...
               if(ntime /= 999999) then
                ...
                if(associated(outField)) deallocate(outField)
...
!              endif
               ...
               if(hintrp) then
                  ...
                  jn_e = jn
                  kn_e = kn
                  if(associated(ofn)) deallocate(ofn)
...
!                   print *,' udflag: ',udflag,' vdflag: ',vdflag
                    ...
                    elseif(u_flag) then
                      if(associated(uofn)) deallocate(uofn)
                      allocate(uofn(in_e,jn_e,kn_e), stat = rc)
                      uvar = outVars(iv)
                      uofn = ofn
                      u_flag = .false.
                    elseif(v_flag) then
                      if(associated(vofn)) deallocate(vofn)
...
!            Write the Attributes.
         ...
         if(iff == 1 .and. it == 1 ) then
             if(associated(bkOut)) deallocate(bkOut)
             if(associated(akOut)) deallocate(akOut)
...
!         print *,' ks_e.ks32 ',ks_e,ks32
          if(associated(pk2)) deallocate(pk2)
          if(associated(pk1)) deallocate(pk1)
          if(associated(pe1)) deallocate(pe1)
          if(associated(pk3d_m)) deallocate(pk3d_m)
...
!    WORK AREA
           ...
           real               :: packing_range_prs(2, mVars)
           real               :: undef
           real, pointer      :: lon_e(:)           ! latitudes in deg (jm)
           real, pointer      :: lat_e(:)           ! latitudes in deg (jm)
           real, pointer      :: ps1(:,:)           ! Surface pressure
           real, pointer      :: delp1(:,:,:)       ! Delp
           real, pointer      :: gaus_ps(:,:)       ! ps
           real, pointer      :: gaus_delp(:,:,:)   ! ps
           real, pointer      :: pe_ec(:,:,:)       ! EC Pressure edges.
           real*8, pointer    :: lev_e(:)           ! levels in eta (km)
           real               :: ak55(:),bk55(:)
           ...
           integer            :: ngatts             ! Number of attributes for GFIO
           integer            :: nVars_e,ihms,jm_old,jme_tmp       
...
!    ------------------
          ...
          if(associated(hhmmss))   deallocate(hhmmss)
          if(associated(yyyymmdd)) deallocate(yyyymmdd)
          if(associated(pe_ec)) deallocate(pe_ec)
...
!             call minmaxpe(delp,im_e,jm_e,kmre)
            ...
            if ( lon_e(1) .lt. 0 ) then ! [-180,180] to [0,360]
             if(associated(delp1)) deallocate(delp1)
         ...
         if(associated(kmVar_e))   deallocate(kmVar_e,stat=rc)
         if(associated(lev_e))     deallocate(lev_e,stat=rc)
         if(associated(lat_e))     deallocate(lat_e,stat=rc)
         if(associated(lon_e))     deallocate(lon_e,stat=rc)
         if(associated(hhmmss))    deallocate(hhmmss,stat=rc)
         if(associated(yyyymmdd))  deallocate(yyyymmdd,stat=rc)
         if(associated(pe_ec))  deallocate(pe_ec,stat=rc)
         if(associated(gaus_ps))  deallocate(gaus_ps,stat=rc)
         if(associated(gaus_delp))  deallocate(gaus_delp,stat=rc)
...
print *
print *, "-------------------------------------------------------------------"
print *, "GFIO_remap - Remaping the data.                                    "
print *, "-------------------------------------------------------------------"
print *
...
print *
print *, "SYNOPYSIS"
print *, "   GFIO_remap [options] input_fname(s)"
print *
print *, "OPTIONS"
print *
...
print *, "  -shift            Shifts the longitude from (-180,180) to (0,360) or viceversa "
print *, "                    for the output fields."
...
print *, "  -psfile    psFile   ana.eta file name (Needed only when the delp is missing from the input file) "
print *, "                      ( Provide an eta file where the delp resides.) "
print *
print *, "DESCRIPTION"
print *, "  Remaps the input data to user specified resolution."
print *
print *, "   ex:  GFIO_remap.x -o outfile input_files"
print * 
...
!
!
! !ROUTINE:  z_interp --- Remaping the data.
!
! !INTERFACE:
!
