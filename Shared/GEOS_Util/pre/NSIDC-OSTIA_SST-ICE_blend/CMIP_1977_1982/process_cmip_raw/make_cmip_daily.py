...
purpose is to :
               ...
               create time interpolated (linearly) daily data [as nc] files.
'''
#-----------------------------------------------------------------
import os
import sys
...
while( proc_day <= end_day):
	...
	# dump interpolated SST(X) & SIC(Y) to a netCDF file
	write_cmip_daily( proc_day, lon_merra, lat_merra, X, Y, iDebug, iPlot_polar_stereo)
