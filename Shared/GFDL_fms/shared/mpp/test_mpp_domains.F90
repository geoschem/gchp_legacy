...
! Adjoint Dot Test ------------------------------------------
! adjoint dot product test is recommended in double precision
! since it is sensitive to the number of significant digits.
...
!---test 3d vector fields----------------------------------------------------------
  ...
  use mpp_mod,         only : mpp_pe, mpp_npes, mpp_root_pe, mpp_sum
    ...
    type(domain2D) :: domain
    real, allocatable, dimension(:,:,:) :: x, x_ad
    ...
    allocate( x (isd:ied,jsd:jed,nz), x_ad (isd:ied,jsd:jed,nz) )
