!  $Id: MAPL_Cap.F90,v 1.40 2013-01-28 19:23:54 atrayano Exp $
...
! !INTERFACE:
  ...
  subroutine MAPL_CAP(ROOT_SetServices, Name, AmIRoot, RC)
...
!---------------------------------------------------------------------------
   ...
   call ESMF_ConfigGetAttribute(config, value=CoresPerNode, Label="CoresPerNode:", default=min(NPES,8), rc=status)
   VERIFY_(STATUS)
...
! ---------------------------
    ...
    DO I=1, ITEMCOUNT
       if(ItemTypes(I) == ESMF_StateItem_Field) then
          ...
          call ESMF_StateAdd(EXPORTS(EXTDATA), field, rc=status)
          VERIFY_(STATUS)
       end if
    END DO
...
!------------------------
   ...
   call ESMF_GridCompInitialize (   GCS(EXTDATA), IMPORTS(EXTDATA), EXPORTS(EXTDATA), & 
                                    CLOCK,  userRC=STATUS )
   VERIFY_(STATUS)
...
! ------------------------------------------------------
      ...
      call ESMF_GridCompWriteRestart( GCS(ROOT), IMPORTS(ROOT), EXPORTS(ROOT), &
           CLOCK_HIST,  RC=STATUS )
      VERIFY_(STATUS)

! Run the ExtData Component
! --------------------------

      call ESMF_GridCompRun     ( GCS(EXTDATA), IMPORTS(EXTDATA), EXPORTS(EXTDATA), &
                                  CLOCK, userRC=STATUS )
      VERIFY_(STATUS)
...
! ----------------------
      ...
      call ESMF_TimeGet  ( CurrTime, YY = AGCM_YY, &
                                     ...
                                     S  = AGCM_S, rc=status )
      VERIFY_(STATUS)
      if( MAPL_AM_I_Root() ) write(6,1000) AGCM_YY,AGCM_MM,AGCM_DD,AGCM_H,AGCM_M,AGCM_S
...
!  --------
   ...
   call ESMF_GridCompFinalize( GCS(ROOT),IMPORTS(ROOT),EXPORTS(ROOT),CLOCK     ,userRC=STATUS )
   VERIFY_(STATUS)
   call ESMF_GridCompFinalize( GCS(HIST),IMPORTS(HIST),EXPORTS(HIST),CLOCK_HIST,userRC=STATUS )
   call ESMF_GridCompFinalize( GCS(EXTDATA),IMPORTS(EXTDATA),EXPORTS(EXTDATA),CLOCK,userRC=STATUS )
...
!  ------------------
   ...
   subroutine Perpetual_Clock ( clock, clock_HIST, PERPETUAL_YEAR, PERPETUAL_MONTH, PERPETUAL_DAY, rc )
     ...
     call ESMF_ClockGetAlarm ( clock_HIST, name='PERPETUAL', alarm=PERPETUAL, rc=status )
     VERIFY_(STATUS)
     call ESMF_AlarmRingerOff( PERPETUAL, rc=status )
     VERIFY_(STATUS)
...
! Get the list of current alarms in the clock
    ...
    VERIFY_(STATUS)
    call ESMF_ClockGetAlarmList(clock, alarmListType=ESMF_ALARMLIST_ALL, &
...
!ALT      ESMF_INIT_CHECK_DEEP(ESMF_ConfigGetInit,config,rc)
      ...
      if ( present (label) ) then
         call ESMF_ConfigGetAttribute( config, curVal, label, rc = iret )
      else
         call ESMF_ConfigGetAttribute( config, curVal, rc = iret )
      endif
