!  $Id: CubeToLatLon.F90,v 1.8 2013-02-04 23:59:44 atrayano Exp $
...
!     unfortunately MAPL does not destroy LocationStreams yet 
  ...
  function CubeLatLonCreate( npx, npy, nlon, nlat, lons, lats, rc ) result(Trans)
...
!    write(*,'(i5.5,"x",i5.5,"_c2l_",i5.5,"x",i5.5)') npx,npy,nlon,nlat
  ...
  !-------------------------------------------------
    ...
    if(MAPL_ShmInitialized) then
       call MAPL_AllocNodeArray(Trans%ee1,shape(ee1),rc=STATUS)
       VERIFY_(STATUS)

       Trans%ee1 => MAPL_RemapBounds(ee1,lbound(ee1,1),lbound(ee1,2),lbound(ee1,3))

       call MAPL_AllocNodeArray(Trans%ee2,shape(ee2),rc=STATUS)
       VERIFY_(STATUS)

       Trans%ee2 => MAPL_RemapBounds(ee2,lbound(ee2,1),lbound(ee2,2),lbound(ee2,3))

       call MAPL_AllocNodeArray(Trans%ff1,shape(ff1),rc=STATUS)
       VERIFY_(STATUS)

       Trans%ff1 => MAPL_RemapBounds(ff1,lbound(ff1,1),lbound(ff1,2),lbound(ff1,3))

       call MAPL_AllocNodeArray(Trans%ff2,shape(ff2),rc=STATUS)
       VERIFY_(STATUS)

       Trans%ff2 => MAPL_RemapBounds(ff2,lbound(ff2,1),lbound(ff2,2),lbound(ff2,3))

       call MAPL_AllocNodeArray(Trans%gg1,shape(gg1),rc=STATUS)
       VERIFY_(STATUS)

       Trans%gg1 => MAPL_RemapBounds(gg1,lbound(gg1,1),lbound(gg1,2),lbound(gg1,3))

       call MAPL_AllocNodeArray(Trans%gg2,shape(gg2),rc=STATUS)
       VERIFY_(STATUS)

       Trans%gg2 => MAPL_RemapBounds(gg2,lbound(gg2,1),lbound(gg2,2),lbound(gg2,3))
       ...
       deallocate(ee1,stat=STATUS)
       VERIFY_(STATUS)
       deallocate(ee2,stat=STATUS)
       VERIFY_(STATUS)
       deallocate(ff1,stat=STATUS)
       VERIFY_(STATUS)
       deallocate(ff2,stat=STATUS)
       VERIFY_(STATUS)
       deallocate(gg1,stat=STATUS)
       VERIFY_(STATUS)
       deallocate(gg2,stat=STATUS)
       VERIFY_(STATUS)
...
!JK for conservative interp--------------
  ...
  subroutine C2LInterp(cubsph, latlon, index, weight, misval, transpose)
