...
! Case 0 parameters
      real(REAL8) :: p0 = 3.0
      real(REAL8) :: rgamma = 5.0
      real(REAL8) :: lat0 = pi/2.0 !pi/4.8
      real(REAL8) :: lon0 = 0.0 !pi-0.8

!  pi_shift moves the initial location of the cosine bell for Case 1
      real(REAL8), parameter :: pi_shift = 0.0 !3.0*pi/4.

!  case 9 parameters 
      real(REAL8)  , allocatable :: case9_B(:,:)
      real(REAL8)   :: AofT(2)

!  Validating fields used in statistics
      real(REAL8)  , allocatable :: phi0(:,:,:) ! Validating Field
      real(REAL8)  , allocatable :: ua0(:,:,:)  ! Validating U-Wind
      real(REAL8)  , allocatable :: va0(:,:,:)  ! Validating V-Wind
      real(REAL8)  , allocatable :: gh_table(:), lats_table(:)

!  Initial Conservation statistics ; total mass ; enstrophy ; energy
      real(REAL8)   :: tmass_orig
      real(REAL8)   :: tvort_orig
      real(REAL8)   :: tener_orig

      real(REAL8), parameter :: r1e5=1.E5
...
!
 ...
 ! defOnGrid = -1:null_op, 0:All-Grids, 1:C-Grid, 2:D-Grid, 3:A-Grid, 4:A-Grid then Rotate, 5:D-Grid with unit vectors then Rotate
      ...
      real(REAL8)  ,    intent(INOUT) :: UBar
      real(REAL8) ,      intent(INOUT) ::    u(isd:ied  ,jsd:jed+1)
      real(REAL8) ,      intent(INOUT) ::    v(isd:ied+1,jsd:jed  )
      real(REAL8) ,      intent(INOUT) ::   uc(isd:ied+1,jsd:jed  )
      real(REAL8) ,      intent(INOUT) ::   vc(isd:ied  ,jsd:jed+1)
      real(REAL8) ,      intent(INOUT) ::   ua(isd:ied  ,jsd:jed  )
      real(REAL8) ,      intent(INOUT) ::   va(isd:ied  ,jsd:jed  )
      ...
      real(REAL8)   :: p1(2),p2(2),p3(2),p4(2), pt(2)
      real(REAL8)   :: e1(3),e2(3),ex(3),ey(3)

      real(REAL8)   :: dist, r, r0 
      integer :: i,j,k,n
      real(REAL8) :: utmp, vtmp

      real(REAL8) :: psi_b(isd:ied+1,jsd:jed+1), psi(isd:ied,jsd:jed), psi1, psi2 
...
!
      subroutine init_case(u,v,w,pt,delp,q,phis, ps,pe,peln,pk,pkz,  uc,vc, ua,va, ak, bk,  &
                           ...
                           dry_mass, mountain, moist_phys, shallow_water, hydrostatic, hybrid_z, delz, ze0)

      real(REAL8) ,      intent(INOUT) ::    u(isd:ied  ,jsd:jed+1,npz)
      real(REAL8) ,      intent(INOUT) ::    v(isd:ied+1,jsd:jed  ,npz)
      real(REAL8) ,      intent(INOUT) ::    w(isd:ied  ,jsd:jed  ,npz)
      real(REAL8) ,      intent(INOUT) ::   pt(isd:ied  ,jsd:jed  ,npz)
      real(REAL8) ,      intent(INOUT) :: delp(isd:ied  ,jsd:jed  ,npz)
      real(REAL8) ,      intent(INOUT) ::    q(isd:ied  ,jsd:jed  ,npz, ncnst)

      real(REAL8) ,      intent(INOUT) :: phis(isd:ied  ,jsd:jed  )

      real(REAL8) ,      intent(INOUT) ::   ps(isd:ied  ,jsd:jed  )
      real(REAL8) ,      intent(INOUT) ::   pe(is-1:ie+1,npz+1,js-1:je+1)
      real(REAL8) ,      intent(INOUT) ::   pk(is:ie    ,js:je    ,npz+1)
      real(REAL8) ,      intent(INOUT) :: peln(is :ie   ,npz+1    ,js:je)
      real(REAL8) ,      intent(INOUT) ::  pkz(is:ie    ,js:je    ,npz  )

      real(REAL8) ,      intent(INOUT) ::   uc(isd:ied+1,jsd:jed  ,npz)
      real(REAL8) ,      intent(INOUT) ::   vc(isd:ied  ,jsd:jed+1,npz)
      real(REAL8) ,      intent(INOUT) ::   ua(isd:ied  ,jsd:jed  ,npz)
      real(REAL8) ,      intent(INOUT) ::   va(isd:ied  ,jsd:jed  ,npz)
      real(REAL8) ,      intent(inout) :: delz(is:ie,js:je,npz)
      real(REAL8) ,      intent(inout)   ::  ze0(is:ie,js:je,npz+1)

      real(REAL8) ,      intent(inout) ::   ak(npz+1)
      real(REAL8) ,      intent(inout) ::   bk(npz+1)
      ...
      logical,      intent(IN) :: shallow_water
      logical,      intent(IN) :: hydrostatic
      logical,      intent(IN) :: hybrid_z

      real(REAL8)   ::  tmp(1-ng:npx  +ng,1-ng:npy  +ng,1:nregions)
      real(REAL8)   :: tmp1(1   :npx     ,1   :npy     ,1:nregions)

      real(REAL8)   :: p1(2)      ! Temporary Point
      real(REAL8)   :: p2(2)      ! Temporary Point
      real(REAL8)   :: p3(2)      ! Temporary Point
      real(REAL8)   :: p4(2)      ! Temporary Point
      real(REAL8)   :: pa(2)      ! Temporary Point
      real(REAL8)   :: pb(2)      ! Temporary Point
      real(REAL8)   :: pcen(2)    ! Temporary Point
      real(REAL8)   :: e1(3), e2(3), e3(3), ex(3), ey(3)
      real(REAL8)   :: dist, r, r0, omg, A, B, C
      integer :: i,j,k,nreg,z,zz
      integer :: i0,j0,n0
      real(REAL8)   :: utmp,vtmp,ftmp
      real(REAL8)   :: rk

      integer, parameter :: jm = 5761
      real(REAL8)   :: ll_phi(jm)
      real(REAL8)   ::   ll_u(jm)
      real(REAL8)   ::   ll_j(jm)
      real(REAL8)   ::   cose(jm)
      real(REAL8)   ::   sine(jm)
      real(REAL8)   ::   cosp(jm)
      real(REAL8)   :: ddeg, deg, DDP, DP, ph5
      real(REAL8)   :: myB, myC, yy
      integer   :: jj,jm1

      real(REAL8) :: Vtx, p, w_p
      real(REAL8) :: x1,y1,z1,x2,y2,z2,ang
      ...
      real(REAL8) :: L1_norm
      real(REAL8) :: L2_norm
      real(REAL8) :: Linf_norm
      real(REAL8) :: pmin, pmin1
      real(REAL8) :: pmax, pmax1
      real(REAL8) :: grad(isd:ied  ,jsd:jed,2)
      real(REAL8) :: div0(isd:ied  ,jsd:jed  ) 
      real(REAL8) :: vor0(isd:ied  ,jsd:jed  )
      real(REAL8) :: divg(isd:ied  ,jsd:jed  )
      real(REAL8) :: vort(isd:ied  ,jsd:jed  )
      real(REAL8) :: ztop, rgrav, p00, pturb, zmid, pk0, t00
      real(REAL8) :: dz1(npz), ppt(npz)
      real(REAL8) :: ze1(npz+1), pe1(npz+1)

      integer :: nlon,nlat
      character(len=80) :: oflnm, hgtflnm

! Baroclinic Test Case 12
      real(REAL8) :: eta, eta_0, eta_s, eta_t
      real(REAL8) :: eta_v(npz), press, anti_rot
      real(REAL8) :: T_0, T_mean, delta_T, lapse_rate, n2, zeta, s0
      real(REAL8) :: pt1,pt2,pt3,pt4,pt5,pt6, pt7, pt8, pt9, u1, pt0
      real(REAL8) :: uu1, uu2, uu3, vv1, vv2, vv3
!     real(REAL8) wbuffer(npx+1,npz)
!     real(REAL8) sbuffer(npy+1,npz)
...
! Aqua-planet case: mean SLP=1.E5
         phis = 0.0
         call hydro_eq(npz, is, ie, js, je, ps, phis, r1e5,      &
...
!--------------- end shallow_water cases if/else --------------------------
    endif
!--------------- end shallow_water cases if/else --------------------------
...
! Rankine vortex
!----------------------------
  real(REAL8), intent(in):: ubar ! max wind (m/s)
  real(REAL8), intent(in):: r0   ! Radius of max wind (m)
  real(REAL8), intent(in):: p1(2)   ! center position (longitude, latitude) in radian
  real(REAL8), intent(inout):: u(isd:ied,  jsd:jed+1)
  real(REAL8), intent(inout):: v(isd:ied+1,jsd:jed)
! local:
  real(REAL8):: p2(2), p3(2), p4(2)
  real(REAL8):: e1(3), e2(3), ex(3), ey(3)
  real(REAL8):: vr, r, d2, cos_p, x1, y1
  real(REAL8):: utmp, vtmp
  integer i, j
...
! shift:
  ...
  enddo
     ...
     real(REAL8) function gh_jet(npy, lat_in)
     integer, intent(in):: npy
     real(REAL8), intent(in):: lat_in
     real(REAL8) lat, lon, dp, uu
     real(REAL8) h0, ft
     integer j,jm
...
! Search:
     ...
     real(REAL8) function u_jet(lat)
      real(REAL8) lat, lon, dp
      real(REAL8) umax, en, ph0, ph1
      ...
      real(REAL8), intent(OUT) :: B(isd:ied,jsd:jed)
      real(REAL8) :: myC,yy,myB
      integer :: i,j
...
!     
   ...
   real(REAL8) , intent(INOUT) :: phis(isd:ied  ,jsd:jed  )
   real(REAL8) , intent(IN) :: time_since_start
   real(REAL8) :: tday, amean
   integer :: i,j
!
! Generate B forcing function
!
...
!                                                in Williamson, 1994 (p.16)
       subroutine get_stats(dt, nt, u,v,pt,delp,q,phis, ps, &
                            uc,vc, ua,va, npx, npy, npz, ncnst, ndims, nregions,    &
                            stats_lun, consv_lun, monitorFreq, shallow_water)
         ...
         real(REAL8)   :: L2_norm
         real(REAL8)   :: Linf_norm
         real(REAL8)   :: pmin, pmin1, uamin1, vamin1
         real(REAL8)   :: pmax, pmax1, uamax1, vamax1
         real(REAL4) :: arr_r4(5)
         real(REAL8)   :: tmass0, tvort0, tener0, tKE0
         real(REAL8)   :: tmass, tvort, tener, tKE
         real(REAL8)   :: temp(is:ie,js:je)
         integer :: i0, j0, k0, n0
         integer :: i, j, k, n, iq

         real(REAL8) :: psmo, Vtx, p, w_p
         real(REAL8) :: x1,y1,z1,x2,y2,z2,ang

         real(REAL8)   :: p1(2), p2(2), p3(2), r, r0, dist, heading

         real(REAL8) :: uc0(isd:ied+1,jsd:jed  ,npz)
         real(REAL8) :: vc0(isd:ied  ,jsd:jed+1,npz)

         real(REAL8) :: myDay, ndays
         integer :: myRec, maxnt

         if (shallow_water) then
           ndays = 12
           if (test_case==5) ndays = 15
           if (test_case==6) ndays = 30
           maxnt = ndays*86400.0/dt
         endif
...
! Get UV Stats
 ...
 203  format(i6.6,A,i6.6,A,e10.4)
 204  format('          ',A,e10.4,' ',e10.4,' ',i4.4,'x',i4.4,'x',i4.4,'x',i4.4)
 205  format('          ',A,e10.4,' ',e10.4,' ',i4.4,'x',i4.4,'x',i4.4,'x',i4.4,' ',e10.4)
 206  format('          ',A,i3.3,A,e10.4,' ',e10.4,' ',i4.4,'x',i4.4,'x',i4.4,'x',i4.4)
...
! Surface Pressure
     ...
     if (gid == masterproc) then
        write(*,204) 'PS   MAX|MIN      : ', 0.01*pmax, 0.01*pmin, i0, j0, n0
     endif
...
! Get PT Stats
         ...
         enddo
         if (gid == masterproc) then
             write(*,204) 'PT   MAX|MIN      : ', pmax1, pmin1, i0, j0, k0, n0
         endif
     ...
     if(gid==masterproc) write(6,*) ' '
         do k=1,npz
            ...
            pmax1 = max(pmax, pmax1)
            if (gid == masterproc) then
                write(*,205) 'PT   MAX|MIN      : ', pmax1, pmin1, i0, j0, k, n0, 0.5*( (ak(k)+ak(k+1))/1.e5 + bk(k)+bk(k+1) )
            endif
         enddo
     if(gid==masterproc) write(6,*) ' '
#endif
...
! Get DELP Stats
         ...
         enddo
         if (gid == masterproc) then
             write(*,204) 'Delp MAX|MIN      : ', pmax1, pmin1, i0, j0, k0, n0
         endif
...
! Get UV Stats
         ...
         enddo
         if (gid == masterproc) then
             write(*,204) 'U    MAX|MIN      : ', uamax1, uamin1, i0, j0, k0, n0
         endif
         ...
         enddo
         if (gid == masterproc) then
             write(*,204) 'V    MAX|MIN      : ', vamax1, vamin1, i0, j0, k0, n0
         endif
...
! Get Q Stats
         ...
         enddo
         if (gid == masterproc) then
             write(*,204) 'Q    MAX|MIN      : ', pmax1, pmin1, i0, j0, k0, n0
         endif
...
! Get tracer Stats
       do iq=2,ncnst
         ...
         enddo
         if (gid == masterproc) then
             write(*,206) 'TR',iq-1,' MAX|MIN      : ', pmax1, pmin1, i0, j0, k0, n0
         endif
       enddo
     endif ! shallow_water
...
! Conservation of Potential Enstrophy
      ...
      enddo
         ...
         if (gid == masterproc) then
            myRec = 1 !myDay*86400.0/dtout + 1
            if (shallow_water) myRec = nt+1
            if(consv_lun > 0) write(consv_lun,rec=myRec) arr_r4(1:4)
         endif
         if ( (gid == masterproc) .and. MOD(nt,monitorFreq)==0) then
...
!                                 Arrival Point (Lon/Lat)
         ...
         real(REAL8) , intent(IN)  :: p1(2), p2(2)
         real(REAL8) , intent(IN)  :: dist
         real(REAL8) , intent(IN)  :: heading
         real(REAL8) , intent(OUT) :: p3(2)
...
!                     for any var
       ...
       subroutine get_scalar_stats(var, varT, npx, npy, ndims, nregions, &
         ...
         integer,      intent(IN) :: ndims
         integer,      intent(IN) :: nregions
         real(REAL8)  ,    intent(IN) ::  var(isd:ied,jsd:jed)
         real(REAL8)  ,    intent(IN) :: varT(isd:ied,jsd:jed)
         real(REAL8)  ,   intent(OUT) :: vmin
         real(REAL8)  ,   intent(OUT) :: vmax
         real(REAL8)  ,   intent(OUT) :: L1_norm
         real(REAL8)  ,   intent(OUT) :: L2_norm
         real(REAL8)  ,   intent(OUT) :: Linf_norm
         ...
         real(REAL8)   :: varSUM, varSUM2, varMAX
         real(REAL8)   :: gsum
         real(REAL8)   :: vminT, vmaxT, vmeanT, vvarT
         integer :: i0, j0, n0
...
!                     for any var
       ...
       subroutine get_vector_stats(varU, varUT, varV, varVT, &
         ...
         integer,      intent(IN) :: ndims
         integer,      intent(IN) :: nregions
         real(REAL8)  ,    intent(IN) ::  varU(isd:ied,jsd:jed)
         real(REAL8)  ,    intent(IN) :: varUT(isd:ied,jsd:jed)
         real(REAL8)  ,    intent(IN) ::  varV(isd:ied,jsd:jed)
         real(REAL8)  ,    intent(IN) :: varVT(isd:ied,jsd:jed)
         real(REAL8)  ,   intent(OUT) :: vmin
         real(REAL8)  ,   intent(OUT) :: vmax
         real(REAL8)  ,   intent(OUT) :: L1_norm
         real(REAL8)  ,   intent(OUT) :: L2_norm
         ...
         real(REAL8)   ::  var(isd:ied,jsd:jed)
         real(REAL8)   :: varT(isd:ied,jsd:jed)
         real(REAL8)   :: vmean
         real(REAL8)   :: vvar
         real(REAL8)   :: vmin1
         real(REAL8)   :: vmax1
...
!
       ...
       real(REAL8), intent(IN) :: ndt
       integer, intent(IN) :: n_split
       integer, intent(IN) :: npx, npy, npz
       logical, OPTIONAL, intent(IN) :: noPrint
       real(REAL8) ,      intent(IN) ::   uc(isd:ied+1,jsd:jed  ,npz)
       real(REAL8) ,      intent(IN) ::   vc(isd:ied  ,jsd:jed+1,npz)
 
       real(REAL8) :: ideal_c=0.06
       real(REAL8) :: tolerance= 1.e-3
       real(REAL8) :: dt_inc, dt_orig 
       real(REAL8)   :: meanCy, minCy, maxCy, meanCx, minCx, maxCx
...
!
      subroutine pmxn(p, npx, npy, nregions, pmin, pmax, i0, j0, n0)
         ...
         integer,      intent(IN) :: npy
         integer,      intent(IN) :: nregions
         real(REAL8)  , intent(IN)  :: p(isd:ied,jsd:jed)
         real(REAL8)  , intent(OUT) :: pmin
         real(REAL8)  , intent(OUT) :: pmax
...
!
      ...
      real(REAL8), allocatable :: tmp(:,:,:)
      real(REAL8), allocatable :: tmpA(:,:,:)
      real(REAL8), allocatable :: ut(:,:,:,:)
      real(REAL8), allocatable :: vt(:,:,:,:)
      real(REAL8), allocatable :: tmpA_3d(:,:,:,:)
      real(REAL8), allocatable :: vort(:,:)
      ...
      real(REAL8)   :: p4(2)      ! Temporary Point
      real(REAL8)   :: pa(2)      ! Temporary Point
      real(REAL8)   :: utmp, vtmp, r, r0, dist, heading
...
!
      ...
      real(REAL8)   ::  tmp(1-ng:npx  +ng,1-ng:npy  +ng,1:nregions)
      real(REAL8)   :: tmpA(1-ng:npx-1+ng,1-ng:npy-1+ng,1:nregions)
      real(REAL8)   :: p1(2)      ! Temporary Point
      real(REAL8)   :: p2(2)      ! Temporary Point
      real(REAL8)   :: p3(2)      ! Temporary Point
      real(REAL8)   :: p4(2)      ! Temporary Point
      real(REAL8)   :: pa(2)      ! Temporary Point
      real(REAL8)   :: ut(1:npx,1:npy,1:nregions)
      real(REAL8)   :: vt(1:npx,1:npy,1:nregions)
      real(REAL8)   :: utmp, vtmp, r, r0, dist, heading
...
#include <netcdf.inc>
         ...
         real(REAL8)  , intent(IN)  :: p(npx-1,npy-1,npz,ntiles)
         integer,      intent(IN) :: ndims

         integer :: error
         real(REAL4), allocatable :: p_R4(:,:,:,:)
         integer :: i,j,k,n
...
!
      subroutine wrt2d(iout, nrec, i1,i2, j1,j2, npx, npy, nregions, p)
         ...
         integer,      intent(IN) :: npy
         integer,      intent(IN) :: nregions
         real(REAL8)  , intent(IN)  :: p(npx-1,npy-1,nregions)

         real(REAL4) :: p_R4(npx-1,npy-1,nregions)
         integer :: i,j,n
...
!
      subroutine init_double_periodic(u,v,w,pt,delp,q,phis, ps,pe,peln,pk,pkz,  uc,vc, ua,va, ak, bk,  &
        ...
        real(REAL8), dimension(is:ie):: pm, qs
        real(REAL8) :: dist, r0, f0_const, prf, rgrav
        real(REAL8) :: ptmp, ze, zc, zm
        real(REAL8) :: t00, p00, xmax, xc, xx, yy, zz, pk0, pturb, ztop
        real(REAL8) :: ze1(npz+1)
         real(REAL8):: dz1(npz)
