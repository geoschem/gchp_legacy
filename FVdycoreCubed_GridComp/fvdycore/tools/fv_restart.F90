module fv_restart_mod
...
#else
  use MAPL_MOD,            only: MAPL_KAPPA, MAPL_PI_R8, &
                                 MAPL_OMEGA, MAPL_RGAS, MAPL_GRAV
#endif

  use fv_arrays_mod,       only: fv_atmos_type, REAL8
#ifndef MAPL_MODE
  ...
  character(len=128) :: tagname = '$Name: Heracles-UNSTABLE $'
...
contains 
  ...
  subroutine fv_restart(fv_domain, Atm, dt_atmos, seconds, days, cold_start, grid_type)
    ...
    real(REAL8), allocatable :: uc(:,:,:), vc(:,:,:)
    real(REAL8), allocatable :: ua(:,:,:), va(:,:,:)

    integer :: unit
    real(REAL8), allocatable :: dz1(:)
    real(REAL8) rgrav, f00, ztop
    logical :: hybrid
...
! Notes by Jeff D.
  ...
  ! Shouldn't have read for all tiles then loop over tiles
    ...
    do n = 1, ntileMe
      ...
      ! Init model data
      if(.not.cold_start)then  ! This is not efficient stacking if there are real(REAL8)ly more tiles than 1.
       ...
       if ( Atm(n)%make_hybrid_z ) then
         ...
         if (grid_type < 4) then
            ...
            call init_case(Atm(n)%u,Atm(n)%v,Atm(n)%w,Atm(n)%pt,Atm(n)%delp,Atm(n)%q,Atm(n)%phis, Atm(n)%ps,Atm(n)%pe, &
                           ...
                           Atm(n)%moist_phys, Atm(n)%shallow_water, Atm(n)%hydrostatic, hybrid, Atm(n)%delz, Atm(n)%ze0)
            endif
