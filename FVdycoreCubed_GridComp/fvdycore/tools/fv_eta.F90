module fv_eta_mod
...
#else
 use MAPL_MOD, only: MAPL_KAPPA, MAPL_GRAV, MAPL_CP, MAPL_RGAS
#endif
 ...
 subroutine set_eta(km, ks, ptop, ak, bk)
      ...
      real(REAL8), intent(out):: ak(km+1)
      real(REAL8), intent(out):: bk(km+1)
      real(REAL8), intent(out):: ptop         ! model top (Pa)
! local
      real(REAL8) a24(25),b24(25)            ! GFDL AM2L24
      real(REAL8) a26(27),b26(27)            ! Jablonowski & Williamson 26-level
      real(REAL8) a32(33),b32(33)
      real(REAL8) a32w(33),b32w(33)
      real(REAL8) a47(48),b47(48)
      real(REAL8) a48(49),b48(49)
      real(REAL8) a52(53),b52(53)
      real(REAL8) a54(55),b54(55)
      real(REAL8) a56(57),b56(57)            ! For Mars GCM
      real(REAL8) a60(61),b60(61)
      real(REAL8) a64(65),b64(65)
      real(REAL8) a100(101),b100(101)
      real(REAL8) a104(105),b104(105)

#if defined(MARS_GCM)
      real(REAL8):: p0= 6.0E2
      real(REAL8):: pc= 1.0e-3
      real(REAL8) a36(37),b36(37)            ! For Mars GCM
      real(REAL8) a28(29),b28(29)            !  Mars 28 levels 
      real(REAL8) a30(31),b30(31)            !  Mars 31 levels
      real(REAL8) a46(47),b46(47)            !  Mars 46 levels 
#elif defined(VENUS_GCM)
      real(REAL8):: p0= 92.E5
      real(REAL8):: pc= 20.E5
      real(REAL8) a50(51), b50(51)
#else
...
! One pressure layer
 ...
 subroutine get_eta_level(npz, p_s, pf, ph, ak, bk, pscale)
  ...
  real(REAL8), intent(in)  :: p_s            ! unit: pascal
  real(REAL8), intent(in)  :: ak(npz+1)
  real(REAL8), intent(in)  :: bk(npz+1)
  real(REAL8), intent(in), optional :: pscale
  real(REAL8), intent(out) :: pf(npz)
  real(REAL8), intent(out) :: ph(npz+1)
  integer k
 ...
 subroutine compute_dz(km, ztop, dz)
  ...
  real(REAL8),   intent(in):: ztop        ! try 50.E3
  real(REAL8),   intent(out):: dz(km)
!------------------------------
  real(REAL8) ze(km+1), dzt(km)
  integer k
...
! Top:
 ...
 subroutine compute_dz_L32(km, ztop, dz)
  ...
  integer, intent(in):: km
  real(REAL8),   intent(out):: dz(km)
  real(REAL8),   intent(out):: ztop        ! try 50.E3
!------------------------------
  real(REAL8) dzt(km)
  real(REAL8) ze(km+1)
  real(REAL8) dz0, dz1, dz2
  real(REAL8) z0, z1, z2
...
!       endif
 ...
 subroutine compute_dz_L101(km, ztop, dz)
  ...
  real(REAL8),   intent(out):: dz(km)
  real(REAL8),   intent(out):: ztop      ! try 30.E3
!------------------------------
  real(REAL8) ze(km+1)
  real(REAL8) dz0, dz1
  real(REAL8):: stretch_f = 1.16
  integer k, k0, k1
...
!  ztop (km) = 20.2859154
 ...
 real(REAL8), intent(in):: rgrav, ztop
 real(REAL8), intent(in):: dz(km)       ! Reference vertical resolution for zs=0
 real(REAL8), intent(in):: hs(is-ng:ie+ng,js-ng:je+ng)
 real(REAL8), intent(inout)::  ze(is:ie,js:je,km+1)
 real(REAL8), optional, intent(out):: dz3(is:ie,js:je,km)
! local
 logical:: filter_xy = .false.
 real(REAL8), allocatable:: delz(:,:,:)
 integer ntimes
 real(REAL8) zint
 real(REAL8):: z1(is:ie,js:je)
 real(REAL8):: z(km+1)
 real(REAL8) sig, z_rat
...
!--------------------------------------
  ...
  real(REAL8), intent(inout):: ze(is:ie,js:je,km+1)
! local:
  real(REAL8), parameter:: df = 0.25
  real(REAL8) dz(km)
  real(REAL8) flux(km+1)
  integer k, n, k1, k2
  ...
  integer, intent(in):: km
  real(REAL8),    intent(in):: p0, ztop
  real(REAL8),    intent(inout):: ptop
  real(REAL8),    intent(inout):: ak(km+1), bk(km+1)
  real(REAL8), intent(out):: pt1(km)
! Local
  logical:: isothermal
  real(REAL8), dimension(km+1):: ze, pe1, pk1
  real(REAL8), dimension(km):: dz1
  real(REAL8) t0, n2, s0
  integer  k
...
! Compute volume mean potential temperature with hydrostatic eqn:
  ...
  real(REAL8), intent(inout):: q(im,km)
!---
  integer i, k
  real(REAL8) qtmp
