...
!
  ...
  ! Hook for the ESMF
  TYPE GIGC_Wrap
     TYPE(GIGC_State), POINTER :: PTR => null()   ! Ptr to GIGC_State
  END TYPE GIGC_Wrap
...
! Locals
    ...
    TYPE(GIGC_State), POINTER     :: myState        ! Legacy state
    TYPE(GIGC_Wrap)               :: wrap           ! Wrapper for myState
...
!!         RC=STATUS  )
   ...
   CALL ESMF_UserCompSetInternalState( GC, 'GIGC_State', wrap, STATUS )
   VERIFY_(STATUS)
...
!    ENDIF
    ...
    write( logLun, '(a,9i6)' ) '### BOB INIT: ', mypet, IM, JM, IM_WORLD, &
                                      JM_WORLD, I_LO, J_LO, I_HI, J_HI
...
!    write( logLun, '(a,9i6)' ) '### BOB RUN: ', mypet, IM, JM, IM_WORLD, &
!                               JM_WORLD, I_LO, J_LO, I_HI, J_HI
...
! 
    ...
    CALL ESMF_UserCompGetInternalState( GC, 'GIGC_State', wrap, STATUS )
    VERIFY_(STATUS)
    myState => wrap%ptr
